...
use cosmwasm_std::{
    coins, to_json_binary, Addr, Api, Binary, BlockInfo, CustomQuery, Empty, Querier, QuerierResult,
    StdError, Storage,
};
use cw_multi_test::{
...
impl Module for TokenFactoryModule {
        ...
        match msg {
            TokenFactoryMsg::CreateDenom { subdenom, metadata } => {
                ...
                if ADMIN.may_load(storage, &new_token_denom)?.is_some() {
                    return Err(anyhow::anyhow!(ContractError::TokenExists.to_string()));
                }
            ...
            TokenFactoryMsg::MintTokens {
                ...
                let admin = ADMIN
                    ...
                    .ok_or_else(|| StdError::generic_err(ContractError::TokenDoesntExist.to_string()))?;
                if admin != sender {
                    return Err(anyhow::anyhow!(ContractError::NotTokenAdmin.to_string()));
                }
    ...
    where
        ...
        QueryC: CustomQuery + DeserializeOwned + 'static,
    {
        Err(anyhow::anyhow!("sudo not implemented for TokenFactory"))
    }
    ...
    fn query(
        ...
        match request {
            ...
            TokenFactoryQuery::Metadata { denom } => {
                ...
                to_json_binary(&MetadataResponse { metadata }).map_err(Into::into)
            }
            TokenFactoryQuery::Admin { denom } => {
                let admin = ADMIN.load(storage, &denom)?.to_string();
                to_json_binary(&AdminResponse { admin }).map_err(Into::into)
            }
            TokenFactoryQuery::DenomsByCreator { creator } => {
                ...
                let denoms = DENOMS_BY_CREATOR
                    ...
                    .unwrap_or_default();
                to_json_binary(&DenomsByCreatorResponse { denoms }).map_err(Into::into)
            }
...
mod tests {
    ...
    // TODO: Update this test for CosmWasm v3 query API changes
    // The query_all_balances method, error downcasting, and Coin API have changed significantly
    // in CosmWasm v3 and need to be updated to the new API
    /*
    #[test]
    fn mint_token() {
